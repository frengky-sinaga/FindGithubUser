...
class FollowingFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        rvUsers = view.findViewById(R.id.rv_followingFragment)
        init()
        showRecycleList()
        followViewModel()
    }
    ...
    private fun init() {
        ...
        sharedViewModel.getDataItem().observe(viewLifecycleOwner, Observer { data ->
            followViewModel.setRequestApi(data.login.toString(), false)
        })
    }
    ...
    private fun followViewModel() {
        followViewModel.getDataFollowing().observe(viewLifecycleOwner, { dataItem ->
            if (dataItem != null) {
                adapter.clearUser()
